...
4. 闭包必须携带 I/O 契约引用（`io_contract_ref`），能力包复制并锁定同一契约，程序与契约对账，形成“程序接口 == 闭包输入契约 == 实际载荷”的单一事实来源。
...
```text
SchemaRef = {
  content_ref: ResourceRef,         // content://sha256/<digest>
  schema_digest: Digest,
  dialect: "json-schema-2020-12-loom-v1"
}

IoContract = {
  schema_version: "io.v1",
  input_mode: "json" | "none",
  output_mode: "json" | "none",
  input_schema_ref: SchemaRef | null,
  output_schema_ref: SchemaRef | null,
  success_semantics: SuccessSemantics | null,
  success_validator_ref: ResourceRef | null,
  input_schema_digest: Digest | null,
  output_schema_digest: Digest | null,
  io_contract_digest: Digest
}
```

- **wire 格式**：JSON Schema 2020-12 的明确子集：`type` / `required` / `properties` / `items` / `enum` / `const` / `minimum` / `maximum` / `format`。`format` 在本版本按 assertion 执行；不支持的关键字拒绝，不静默忽略。
- **内容寻址**：`io_contract_digest` 是权威身份；schema 正文必须先写入 MinIO/S3 ContentStore，得到 `content://sha256/<digest>`，闭包/能力包只保存 `SchemaRef` 和 digest。schema 使用 UTF-8 的 RFC 8785/JCS 规范化 JSON；`io_contract_digest` 对规范化的 `schema_version + schema refs/digests + success_semantics + success_validator_ref` 计算 SHA-256，排除 digest 自身；`ResourceRef.access_binding` 不参与任何契约 digest。
- **身份不变量**：`input_mode=none` 时 input schema/ref/digest 必须为 null，`input_mode=json` 时三者必须存在；output 同理。`SchemaRef.schema_digest` 必须等于 `content_ref.version_or_digest`；`IoContract.input_schema_digest`/`output_schema_digest` 必须分别等于对应 `SchemaRef` 的 digest；所有 digest 不一致都返回结构化 conflict，不允许选择“以内联内容为准”。
- **写入规则**：schema 必须先通过 ContentStore 写入接口取得 `SchemaRef`，再在 patch 中引用；Observer 校验 ref 与 digest 一致，持久化模型中不存在 inline schema。
- **契约字段**：`ProgramApplication.input_schema` / `output_schema` 改为 `SchemaRef | null`（描述性字符串不再进入可执行路径）；可执行 Node 必须声明 `IoContract`。无输入/无 JSON 输出必须显式使用 `null` 并声明对应 `schema_mode=none`，不能依赖“缺字段即兼容”。

`ProgramApplication.success_semantics` 保留 v2 语义字段，但在 `io.v1` 中必须与 `IoContract.success_semantics` 完全相等；后者是校验和 digest 的唯一权威，前者只是闭包应用视图。无输入/无 JSON 输出使用 `input_mode=none` / `output_mode=none`。

无输入/无 JSON 输出必须显式使用 `null` 并声明对应的 `input_mode=none` / `output_mode=none`，不能依赖“缺字段即兼容”。

文中早先出现的 `schema_mode=none` 与上述 `input_mode=none` / `output_mode=none` 同义，以后者为规范字段。
...
SuccessSemantics = {
  ...
  subject: "validated_output_json",
  parameters: StructuredValue
}
```

`parameters` 的具体语义只由被引用的 plugin 版本定义，不能由 Observer 根据任意 `op/field/value` 字段自行推断。

```text
ValidationPluginDescriptor = {
  plugin_ref: ResourceRef,
  plugin_digest: Digest,
  api_version: "validator.v1",
  input: "validated_output_json"
}
...
plugin 引用和 digest 属于闭包契约的一部分，并由能力包原样锁定。没有 plugin 时，输出 schema 仍可校验，但成功标准只能进入 `attestation_required`，由 coding-agent 读取证据后调用 `attest_criterion`；Observer/Slave 不猜测业务语义。
...
1. 若闭包声明了 `input_schema_ref`：
   - 节点必须有已绑定的输入 `ResourceRef`；缺失 → blocker `payload_missing`；
   - Observer 通过 ContentStore 取回内容并校验其 digest，再校验内容对 `input_schema_ref`；
...
2. `bind_compute_hole` 时若 binding 引用能力包：能力包的 `io_contract_digest` 必须与闭包一致，并且 input/output schema、success semantics、success validator ref 全部严格相等；不匹配 → blocker `io_contract_mismatch`。
3. `commit` / `start` 复用同一 `_evaluate_readiness`，保证“未通过结构校验不得 ready / commit / start”（对应 v2 “平台不伪造 ready”）。

### 6.2 执行后校验（terminal / evidence）

1. Slave 在 executor 返回 `value` 后，按包自带的 `output_schema_ref` 取回并校验输出 JSON：
   - 通过 → 生成 `ValidationEvidence`（记录 schema digest、结果 digest、validator 版本）；
...
3. Observer 在 `record_result` 接受前按同一 schema/plugin digest 复核；校验失败时接受失败 Attempt/ExecutionOutcome，并将 Run 投影为 `decision_required(validation_failed)`，不进入 `completed`。验证通过只产生 evidence，Run 是否 completed 仍按 v2 的 required criteria 全量重算。
...
Observer 必须校验 attempt/execution/epoch、schema/plugin digest 和输入输出 digest；旧 epoch 或其他 Attempt 的 evidence 一律拒绝。
...
- 包的程序正文若带 `__loom_contract__`（程序自声明接口）则与 `io_contract` 做等值校验；不一致 → `program_contract_mismatch`。读取只允许对源码做 AST 字面量提取，禁止 import、执行或调用包代码；动态生成或无法静态提取的声明视为缺失。
- `__loom_contract__` 的规范化内容纳入 package digest；更推荐把声明放在独立 manifest 中，通过 `ResourceRef` 引用，程序正文只作为待执行内容。
...
- `ProgramApplication`：保留 v2 的 `input_schema` / `output_schema` 语义字段，但类型固定为 `SchemaRef | null`，并新增 `io_contract_ref`；不再把描述字符串作为可执行契约。
- 新增 `IoContract` 模型（见 §5），内容寻址。
- `CapabilityPackageVersion`：新增 `io_contract_ref` / `io_contract_digest` / `input_schema_digest` / `output_schema_digest`，并锁定 `success_validator_ref`（如有）；与 v2 `CapabilityDescriptor` 的 schema digest 对齐。
- `NodeInputBinding`：`node_id`、`input_ref: ResourceRef`、`content_digest`、`schema_digest`、`provenance`；执行输入不再存为裸 `metadata.execution_payload`。
- 新增 migration：上述字段的 JSONB 列。

### 8.2 MCP / 工具描述

- 新增 `loom_put_schema` 数据面操作：上传规范化 JSON Schema 到 ContentStore，返回 `SchemaRef`；schema 正文不嵌入 `IoContract`、patch、事件或日志。
- `materialize_capability_package_candidate`：接收 `io_contract_ref`；工具描述说明“可执行 realization 必须声明 I/O 契约”。schema 正文必须预先通过 ContentStore 写入接口取得 ref。
...
- 新增 `loom_v2/contracts/io_schema.py`：纯函数 `validate(schema, value) -> list[ValidationError]`（实现 JSON Schema 2020-12 的 `json-schema-2020-12-loom-v1` 子集）；Observer 与 Slave 复用同一实现，并在 evidence 中记录 validator digest。
- schema 正文入 MinIO/S3 版 ContentStore（异步 `put`，身份由内容推出，`GET /api/v1/content/{digest}` 可回读）；事件/闭包只存 `SchemaRef`/digest，`access_binding` 不携带部署位置。
- 本阶段不扩展 validator 的资源限制/复杂度治理；只允许文档列出的关键字、禁止 `$ref` 和类型 coercion，后续再单独设计复杂 schema 的限额与沙箱。

### 8.4 契约兼容与校验失败状态

- `io.v1` 采用严格兼容：closure 与 package 的 `io_contract_digest`、input/output schema digest、success semantics 和 success validator ref 必须完全相等；不实现未定义的“可参数化”或通用 schema 子集关系。
...
- **校验器单测**（`tests/contracts/test_io_schema.py`）：type/required/properties/items/enum/minimum/maximum/format；多包一层 `{"payload":{...}}` 必须报 `required` 缺 `scores`；schema 的 JCS 规范化序列化/digest 一致性（与 MinIO 版 ContentStore 的 digest 身份一致）。
...
- **包物化测试**（`tests/api/test_capability_packages.py`）：未声明 `io_contract_ref` → `io_contract_required`；`__loom_contract__` 与契约不一致 → `program_contract_mismatch`；绑定契约（input/output/success/plugin）不匹配 → `io_contract_mismatch`。
...
3. 新物化的可执行能力包必须声明 `io_contract_ref`；程序接口与契约对账，绑定包与闭包的 input/output/success/plugin 契约严格一致。
