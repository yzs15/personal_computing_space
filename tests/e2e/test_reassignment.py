...
def test_slave_a_loss_creates_new_attempt_without_new_execution():
    ...
    patched = client.post(
        ...
        json={
            "operation_id": "begin-reassignment",
            "base_draft_version": run["draft_version"],
            "base_snapshot_digest": run["draft_digest"],
            "ops": [],
        },
    ).json()
    committed = client.post(f"/api/v1/runs/{run['run_id']}/commit", json={"draft_version": patched["draft_version"], "draft_digest": patched["draft_digest"]}).json()
